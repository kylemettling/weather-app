...
export default function Home() {
  ...
  function processWeatherClass(weatherItem) {
    const validTypes = ["Current", "Max", "Min"];
    if (validTypes.includes(weatherItem.text)) {
      return weatherItem.value > 85
        ? "hotWeather"
        : weatherItem.value < 60
        ? "coldWeather"
        : "temporateWeather";
    } else if (weatherItem.text === "Humidity") {
      console.log(weatherItem.value.split("%")[0]);
      return Number(weatherItem.value.split("%")[0]) > 80
        ? "hotWeather"
        : Number(weatherItem.value.split("%")[0]) < 50
        ? "coldWeather"
        : "temporateWeather";
    }
  }
        ...
        {weatherResults && (
          <div className="outputGroup">
            ...
            <ul className="weatherOutput">
              {weatherItems.map((item, index) => {
                return (
                  <li className={processWeatherClass(item)} key={index}>
                    {item.value}
                  </li>
                );
              })}
